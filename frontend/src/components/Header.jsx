  ...
  return (
    <header className="sticky top-0 z-30 bg-white border-b-2 border-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        <Link to="/" data-testid="brand-logo" className="flex items-center gap-2 group shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setOpen(!open)} className="xl:hidden w-10 h-10 border-2 border-black rounded-lg grid place-items-center" data-testid="mobile-menu-toggle">
            {open ? <X /> : <Menu />}
          </button>
        </div>
      </div>

      {/* Nav row - its own full-width line below the logo/actions row, part of the
          same sticky header so it stays visible while scrolling. */}
      {user && user !== false && (
        <nav className="hidden xl:flex max-w-7xl mx-auto px-4 sm:px-6 items-center gap-1 border-t-2 border-black h-12">
          {links.map((l) => (
            <NavLink
              key={l.to}
              to={l.to}
              ...
              className={({ isActive }) =>
                `px-3 py-2 rounded-md text-sm font-semibold flex items-center gap-2 whitespace-nowrap transition-colors ${
      ...
      {open && (
        <div className="xl:hidden border-t-2 border-black bg-white px-4 py-3 flex flex-col gap-1">
